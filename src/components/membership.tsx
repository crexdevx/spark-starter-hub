...
const WHATSAPP_NUMBER = "919101376268";

function whatsappUrl(plan: Plan, category: "Men" | "Women") {
  const message = `Hi Sky Lifting Club, I'm interested in the ${category} ${plan.name} membership (${plan.duration}) — total ${plan.total}. Please share the next steps to join.`;
 ...
 * Each Join Now button opens WhatsApp with a pre-filled membership message.
 */
export function Membership() {
  const [tab, setTab] = useState<"men" | "women">("men");
  const [glowCards, setGlowCards] = useState(false);
  const plans = PLANS[tab];
  const categoryLabel = tab === "men" ? "Men" : "Women";
    ...
    >
      <div className="mx-auto max-w-7xl px-4 py-16 sm:px-6 lg:px-8 lg:py-24">
        ...
        <div
          ...
          className="mx-auto mt-10 flex w-full max-w-xs items-center gap-2 rounded-full border-2 border-on-pure p-1"
        >
            ...
            <button
              ...
              className={`flex-1 rounded-full px-4 py-2 text-sm font-bold uppercase tracking-widest transition-colors ${
                tab === key
                  ...
                  : "text-on-pure hover:bg-on-pure/10"
              }`}
            >
              {key === "men" ? "Men" : "Women"}
            </button>
        ...
        >
          {plans.map((plan) => (
            <article
              key={`${tab}-${plan.name}`}
              className={`flex min-w-[17rem] flex-1 snap-start flex-col rounded-2xl bg-membership-card p-6 text-on-membership-card shadow-[0_18px_40px_-18px_rgba(0,0,0,0.6)] ring-offset-2 ring-offset-surface-pure transition-all duration-300 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white/90 active:ring-2 active:ring-white/90 lg:min-w-0 ${
                glowCards ? "ring-2 ring-white shadow-[0_0_30px_8px_rgba(255,255,255,0.45)]" : ""
              }`}
              tabIndex={0}
            >
              ...
              <div className="mt-auto pt-6">
                <p className="text-xs font-bold uppercase tracking-[0.25em]">Total</p>
                <p className="mt-1 font-display text-4xl leading-none">{plan.total}</p>
              </div>

              <a
                href={whatsappUrl(plan, categoryLabel)}
                target="_blank"
        ...
        {/* Additional charges */}
        <div className="mt-12 rounded-2xl border-2 border-on-pure p-6">
          <h3 className="font-display text-xl uppercase tracking-wide">Other Charges</h3>
          <ul className="mt-4 space-y-3 text-sm font-semibold sm:text-base">
            {EXTRAS.map((extra) => (
              <li
                key={extra.label}
                className="flex flex-wrap items-baseline justify-between gap-x-3 border-b border-on-pure/25 pb-2 last:border-0"
              >
                <span className="uppercase tracking-wide">{extra.label}</span>
                <span className="flex items-baseline gap-2">
                  <span className="font-display text-lg">{extra.value}</span>
                  {extra.note ? (
                    <span className="text-xs font-medium opacity-70">({extra.note})</span>
                  ) : null}
                </span>
              </li>
            ))}
